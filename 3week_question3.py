...
user = []
word_list = ['침대', '나무', '갈매기', '한서대학교', '멋쟁이', '사자']
word_data = []
...
count = 0
while len(user) > 1: # user 인원이 1명이 되면 반복문은 종료된다. 그 전까지는 반복한다. 
    user_num = len(user)
    i = user[count % user_num] # 게임을 user 순서대로 진행하기 위해서 count = 0 부터 user의 수로 나눈 나머지로 index를 찾는다. 
    ...
    if len(user) == user_num-1: # 게임을 제게하기 위한 조건 (새로운 탈락자 발생)
